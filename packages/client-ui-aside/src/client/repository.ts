...
export interface AsideRemote {
  list(request: { parentSessionId: string }): Promise<{
    readonly ok: boolean
    readonly value?: { records: AsideRecord[] }
    readonly error?: { readonly message: string }
  }>
}

/** Normalize one record's anchor text for the sidebar display. */
export function asideText(record: AsideRecord, max = 60): string {
...
export class AsideRepository {
  private records: readonly AsideRecord[] = []
  ...
  constructor(private readonly remote: AsideRemote) {}
   ...
   */
  async refresh(parentSessionId: string): Promise<void> {
  ...
  add(record: AsideRecord): void {
    ...
    if (index === -1) this.records = [...this.records, record]
  ...
  find(parentSessionId: string, anchor: AsideAnchor): AsideRecord | undefined {
    ...
    return this.records.find(record => (
      record.parentSessionId === parentSessionId && anchorKey(record.anchor) === key
    ))
  }
