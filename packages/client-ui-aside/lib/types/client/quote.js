...
export function collectTextSpans(root) {
    ...
    if (container.nodeType === Node.ELEMENT_NODE) {
        ...
        for (const span of spans) {
            if (container.compareDocumentPosition(span.node) & Node.DOCUMENT_POSITION_CONTAINED_BY) {
                if ((child.compareDocumentPosition(span.node) & (Node.DOCUMENT_POSITION_FOLLOWING | Node.DOCUMENT_POSITION_CONTAINED_BY)) !== 0)
                    return span.start;
            }
        }
    }
...
function firstTextOffset(spans, root) {
    for (const span of spans) {
        if (root === span.node || root.contains(span.node))
            return span.start;
    }
    return null;
}
 ...
 */
export function buildQuote(messageEl, range) {
...
function spanAt(spans, offset) {
    ...
    if (last !== undefined && offset >= last.end)
 ...
 */
export function findRowContaining(root, exact) {
    ...
    for (const row of root.querySelectorAll('[data-chat-anchor-key]')) {
        if (!(row instanceof HTMLElement))
            continue;
        if (normalizeText(row.textContent ?? '').includes(needle))
            return row;
    }
    return null;
}
 ...
 */
export function restoreInConversation(root, anchor) {
